...
OpenSorSe 1.0 uses a validated JSON settings file of at most 1 MiB in local application data. Implemented groups cover logging, `Features.ShowAdvancedFeatures`, catalog opt-in, optional AI endpoint/model/5–300 second timeout/history reuse, `Ai.Enabled`, independent rename/folder capability flags, raw request diagnostics, bounded metadata/OCR settings, and `SemanticSearch.Enabled` plus document/result limits. AI, Advanced, OCR, raw diagnostics, catalog, and Semantic Search default off; bounded metadata extraction defaults on. Missing 1.0 properties receive safe defaults, so a v0.9.1 file needs no rewrite or deletion and hidden values remain preserved. Content, semantic, history, AI decisions, catalog, and saved searches use separate stores. The only environment override is `OPENSORSE_LOGGING__MINIMUMLEVEL`.
