...
| --- | --- | --- |
| `content-cache.json` | Source fingerprint, bounded metadata, bounded native/OCR text, engine facts | File bytes, embeddings, credentials |
...
Each envelope has an explicit schema version. Temporary siblings are used for atomic replacement. Unsupported or corrupt optional cache/index files produce a controlled rebuild state. Structure history is preserved unless explicitly cleared.
...
flowchart TD
    ...
    CorruptHistory["Corrupt history"] --> Preserve["Report and preserve"]
