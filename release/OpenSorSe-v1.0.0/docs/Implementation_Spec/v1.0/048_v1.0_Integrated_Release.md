...
| Status | Implemented on local `v1.0`; manual GUI verification pending |
...
### Implemented feature classification

Classification follows user complexity and safety needs, not implementation complexity.

| Feature/page | Class | AI | Visibility/availability | Reason |
| --- | --- | --- | --- | --- |
| Dashboard, Scan, Results, Duplicate drawer | Regular | No | Always | Primary scan and review workflow. |
| Saved catalog, Catalog search, Rules | Regular | No | Always; catalog storage remains its own opt-in | Ordinary historical/search/review tools; Rules does not execute. |
| Metadata and OCR settings | Regular | No | Settings; OCR capability remains separately enabled | Local understanding is a primary 1.0 workflow and failure is isolated. |
| Semantic Search Beta | Regular opt-in | No | `SemanticSearch.Enabled` | User-facing search; it does not require AI or Advanced mode. |
| Compare snapshots | Advanced | No | Advanced | Specialist historical comparison with scope/detail controls. |
| Diagnostics and Operation history | Advanced | No | Advanced | Technical troubleshooting and internal record detail. |
| Structure history and diagrams | Advanced | No | Advanced | Specialized organization audit and the only separately confirmed mutation workflow. |
| AI rename/folder suggestion panels | Regular AI | Yes | Global AI plus the independent capability | Narrow review-only assistance can remain available without other advanced UI. |
| Endpoint/model/timeout setup | Regular AI | Yes | Global AI | Essential provider configuration must not be hidden behind Advanced mode. |
| Raw AI request diagnostics and low-level bounds | Advanced AI | Yes | Global AI plus Advanced plus explicit diagnostic opt-in where applicable | Sensitive technical troubleshooting information. |
| Settings, Help, About and critical safety/error states | Regular | No | Always | Required recovery, explanation, and product identity. |

## 6. Metadata and OCR

`IMetadataExtractor` implementations publish bounded typed fields with provenance, confidence, and warnings. `IMetadataExtractionPipeline` combines filesystem facts, safe embedded metadata, and optional native text. `ContentIndexingService` invokes `IOcrService` only when enabled and needed, normalizes the bounded result, generates provenance tags, and atomically updates `IContentStore`.

`IOcrEngine` reports capability and engine identity. `IOcrService` applies settings, native-text skip rules, extension/file/page/text/time bounds, cancellation, and result normalization. `ContentIndexingService` owns cache reuse and source-fingerprint invalidation. OCR states are Pending, Processing, Completed, Skipped, Failed, PartiallyCompleted, Unavailable, and TextNotIndexedDueToBounds. The Tesseract adapter is optional and never required by tests.
...
`ISemanticIndexer` transforms bounded `ContentRecord` values into persisted `SemanticIndexEntry` records with source/index fingerprints and deterministic feature-hashing vectors. Incremental synchronization updates changed files and removes absent files. `ISemanticSearchService` tokenizes the query once and combines explicit signals with cosine similarity.
...
| `content-index.json` | 1 | 2,000 entries / 128 MiB / bounded text | clear/rebuild |
| `semantic-index.json` | 1 | 10,000 documents / bounded vectors | clear/rebuild |
| `structure-history.json` | 1 | 250 runs / 4,000 nodes per snapshot / 64 MiB | controlled empty recovery/explicit clear |
