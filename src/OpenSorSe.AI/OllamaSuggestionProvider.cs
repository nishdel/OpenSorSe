...
public sealed class OllamaSuggestionProvider : IAiSuggestionProvider
{
    ...
    public async Task<AiConnectionResult> GetConnectionAsync(AiSettings settings, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await using var content = await response.Content.ReadAsStreamAsync(requestCancellation.Token).ConfigureAwait(false);
            var payload = await JsonSerializer.DeserializeAsync<ModelsResponse>(content, JsonOptions, requestCancellation.Token).ConfigureAwait(false);
    ...
    public async Task<AiProviderGenerationResult> GenerateAsync(AiProviderGenerationRequest request, CancellationToken cancellationToken)
    {
        ...
        if (!TryCreateEndpoint(request.Endpoint, out var endpoint) || string.IsNullOrWhiteSpace(request.Model) || string.IsNullOrWhiteSpace(request.Prompt))
        {
        ...
        try
        {
            ...
            await using var content = await response.Content.ReadAsStreamAsync(requestCancellation.Token).ConfigureAwait(false);
            var payloadResponse = await JsonSerializer.DeserializeAsync<GenerateResponse>(content, JsonOptions, requestCancellation.Token).ConfigureAwait(false);
