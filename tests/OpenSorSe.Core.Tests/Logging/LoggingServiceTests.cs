...
public sealed class LoggingServiceTests
{
    ...
    public void Initialize_WritesCategorizedEntriesAndStatistics()
    {
        ...
        var filePath = Assert.Single(Directory.GetFiles(directory.Path, "opensorse-*.log"));
    ...
    public void Initialize_FiltersEntriesBelowMinimumLevel()
    {
        ...
        var output = File.ReadAllText(Assert.Single(Directory.GetFiles(directory.Path, "opensorse-*.log")));
    ...
    public void Initialize_AppliesBoundedDailyRetention()
    {
        ...
        {
            File.WriteAllText(Path.Combine(directory.Path, $"opensorse-2024-01-{day:00}.log"), "old");
        }
        ...
        Assert.Equal(7, Directory.GetFiles(directory.Path, "opensorse-*.log").Length);
