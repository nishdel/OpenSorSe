...
namespace TidyMind.Executor.Tests;
...
public sealed class UndoEngineTests
{
    ...
    {
        var services = new ServiceCollection(); services.AddTidyMindCore(new TidyMindCoreOptions { ConfigurationFilePath = Path.Combine(Path.GetTempPath(), "tidymind-test-settings.json") }); services.AddSingleton<IUndoEngine, UndoEngine>();
    ...
    private sealed class Temp : IDisposable { public Temp() { Path = System.IO.Path.Combine(System.IO.Path.GetTempPath(), $"TidyMind.Undo.{Guid.NewGuid():N}"); Directory.CreateDirectory(Path); } public string Path { get; } public string File(string name,string text) { var path=System.IO.Path.Combine(Path,name); System.IO.File.WriteAllText(path,text); return path; } public void Dispose()=>Directory.Delete(Path,true); }
